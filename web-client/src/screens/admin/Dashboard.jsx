...
const STAT_ICONS = {
    ...
    alert: 'M12 9v4M12 17h.01M10.3 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.7 3.86a2 2 0 0 0-3.4 0z',
...
export default function Dashboard() {
    ...
    const { users, usersTotal, bookings, booksTotal, devicesTotal, roomsTotal, auditEntries } =
        data;
    ...
    const overdueItems = bookings.filter(
        (b) => b.status === 'CHECKED_OUT' && b.endAt && new Date(b.endAt).getTime() < now
    ).length;
    const pendingApprovals = bookings.filter((b) => b.status === 'PENDING').length;
    const newMembers = users.filter(
        (u) => u.raw?.createdAt && now - new Date(u.raw.createdAt).getTime() < SEVEN_DAYS_MS
    ).length;
            ...
            >
                {adminStats.map((stat, i) => (
                    ...
                    >
                        ...
                        <div style={{ fontSize: 11, color: stat.trendColor, fontWeight: 600 }}>
                            {stat.trend}
                        </div>
                    </div>
                ))}
