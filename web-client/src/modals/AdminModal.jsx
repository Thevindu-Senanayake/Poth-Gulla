...
export default function AdminModal() {
  ...
  useEffect(() => {
    if (mode === "copies" && copiesBook) {
      getBook(copiesBook.id)
        .then((b) => setCopies(b.raw?.copies || []))
        .catch(() => setCopies([]));
    }
  }, [mode, copiesBook]);

  function close() {
    setAdminModal((m) => ({ ...m, open: false }));
  }
  function setUf(key, val) {
    setAdminModal((m) => ({ ...m, uf: { ...m.uf, [key]: val } }));
  }
  ...
  async function handleSubmit() {
    ...
    try {
      if (mode === "addUser") {
        if (!uf.uname?.trim() || !uf.uemail?.trim())
          throw new Error("Name and email are required");
        if ((password || "").length < 8)
          throw new Error("Password must be at least 8 characters");
        await registerUser({
          email: uf.uemail.trim(),
          name: uf.uname.trim(),
          password,
          role: ROLE_ENUM[uf.urole] || "STUDENT",
        });
        ...
        if (uf.urole === "student" || uf.urole === "lecturer")
  ...
  async function doRetire(copy) {
    try {
      ...
      setCopies((prev) =>
        prev.map((c) => (c.id === copy.id ? { ...c, status: "RETIRED" } : c)),
      );
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <input
                  style={FIELD_STYLE}
                  value={uf.uname || ""}
                  onChange={(e) => setUf("uname", e.target.value)}
                  placeholder="e.g. Kavya Rajapaksa"
                />
              </div>
              <div>
                <label style={LABEL_STYLE}>
                  Email{" "}
                  {mode === "editUser" && (
                    <span style={{ color: "#9b9db2" }}>(read-only)</span>
                  )}
                </label>
                <input
                  type="email"
                  style={{
                    ...FIELD_STYLE,
                    opacity: mode === "editUser" ? 0.6 : 1,
                  }}
                  disabled={mode === "editUser"}
                  value={uf.uemail || ""}
                  onChange={(e) => setUf("uemail", e.target.value)}
                  placeholder="name@iit.ac.lk"
                />
              </div>
              {mode === "addUser" && (
                <div>
              ...
              {mode === "editUser" &&
                (uf.urole === "student" || uf.urole === "lecturer") && (
                  <div>
                    <label style={LABEL_STYLE}>
                      Tier (sets points to tier floor)
                    </label>
                    <select
                      style={{ ...FIELD_STYLE, cursor: "pointer" }}
                      value={tier}
