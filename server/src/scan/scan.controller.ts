...
export class ScanController {
    ...
    @ApiOperation({ summary: 'Staff scan booking QR + asset QR to check out a book or device' })
    @Post('checkout')
    ...
    @ApiOperation({ summary: 'User scans room door QR to confirm attendance (+20 pts)' })
    @Post('room-checkin')
    roomCheckin(@CurrentUser() user: AuthUser, @Body() dto: RoomCheckinDto) {
        return this.scan.roomCheckin(user.userId, dto);
    }

    @ApiOperation({ summary: 'Staff scan asset QR to return a book or device; applies point scoring' })
    @Post('return')
    @Roles(Role.ADMIN, Role.LIBRARY_STAFF)
