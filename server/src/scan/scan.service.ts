...
export class ScanService {
    ...
    async checkout(dto: CheckoutDto): Promise<Booking> {
        ...
        if (!booking) throw new NotFoundException('Invalid booking QR — no matching booking found');
        if (booking.status !== BookingStatus.APPROVED) {
            throw new BadRequestException(`Booking is ${booking.status}, not APPROVED`);
        }

        if (booking.resourceType === ResourceType.BOOK) return this.checkoutBook(booking, dto.assetTag);
        if (booking.resourceType === ResourceType.DEVICE) return this.checkoutDevice(booking, dto.assetTag);
        throw new BadRequestException('Room bookings are checked in via /scan/room-checkin');
    }
    ...
    private async checkoutBook(
        ...
        if (!copy) throw new NotFoundException(`No book copy with asset tag ${assetTag}`);
        if (copy.bookTitleId !== booking.bookTitleId) {
            throw new BadRequestException('Asset tag belongs to a different book title');
        }
    ...
    private async checkoutDevice(
        ...
        if (!device) throw new NotFoundException(`No device with asset tag ${assetTag}`);
        ...
        if (device.status !== ItemStatus.AVAILABLE) {
            throw new BadRequestException(`Device is ${device.status}, not AVAILABLE`);
        }
    ...
    async roomCheckin(userId: string, dto: RoomCheckinDto): Promise<Booking> {
        ...
        if (!booking) {
            throw new BadRequestException('No active approved booking for this room right now');
        }
        ...
        await this.points.applyFixed(userId, 'ROOM_ATTENDED', { bookingId: booking.id });
    ...
    async returnItem(dto: ReturnItemDto) {
        ...
        if (!borrowing) {
            throw new NotFoundException(`No active borrowing for asset tag ${dto.assetTag}`);
        }

        const returnedAt = new Date();
        const daysLate = Math.ceil((returnedAt.getTime() - borrowing.dueAt.getTime()) / DAY_MS);
        ...
        await this.prisma.$transaction([
            this.prisma.borrowing.update({
                ...
                data: { status: BorrowingStatus.RETURNED, returnedAt, condition: dto.condition },
            }),
        ...
        // Apply return points
        await this.points.apply(borrowing.userId, action, delta, { borrowingId: borrowing.id });
