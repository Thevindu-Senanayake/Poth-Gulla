...
import { DEFAULT_SYSTEM_CONFIG, SystemConfigService } from './system-config.service.js';
...
describe('SystemConfigService', () => {
    ...
    it('returns the stored config when the row exists', async () => {
        ...
        prisma.systemConfig.findUnique.mockResolvedValue({ id: 'singleton', data: stored });
    ...
    it('lazily seeds defaults when no row exists', async () => {
        ...
        prisma.systemConfig.create.mockImplementation(async ({ data }: any) => ({ data: data.data }));
        ...
        expect(prisma.systemConfig.create).toHaveBeenCalledWith(
            expect.objectContaining({ data: expect.objectContaining({ id: 'singleton' }) }),
        );
    });
    ...
    it('merges a partial patch over the current config and upserts', async () => {
        ...
        expect(result).toEqual({ tiers: newTiers, penalties: current.penalties, toggles: current.toggles });
