...
export class BookingService {
    ...
    async create(userId: string, dto: CreateBookingDto): Promise<Booking> {
        ...
        const { resourceType, resourceId, startAt: startStr, endAt: endStr, message } = dto;
        const startAt = new Date(startStr);
        const endAt = new Date(endStr);

        // (1) duration window + cap
        const durationMs = endAt.getTime() - startAt.getTime();
        if (durationMs <= 0) throw new BadRequestException('endAt must be after startAt');
    ...
    private async route(
        ...
        if (type === ResourceType.DEVICE) {
            const device = await this.prisma.device.findUnique({ where: { id: resourceId } });
    ...
    async cancel(actorId: string, bookingId: string, adminOverride = false): Promise<Booking> {
    ...
    async findMine(
        userId: string,
        params: { page: number; limit: number; status?: BookingStatus; resourceType?: ResourceType },
