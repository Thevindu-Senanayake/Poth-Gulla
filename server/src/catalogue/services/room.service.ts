...
export class RoomService {
  ...
  async create(dto: CreateRoomDto): Promise<StudyRoom> {
    const exists = await this.prisma.studyRoom.findUnique({
      where: { name: dto.name },
    });
    if (exists)
      throw new ConflictException(`Room "${dto.name}" already exists`);
  ...
  async remove(id: string): Promise<StudyRoom> {
    ...
    if (room.status === ItemStatus.BORROWED) {
      throw new BadRequestException(
        'Cannot remove a room with an active session',
      );
    }
