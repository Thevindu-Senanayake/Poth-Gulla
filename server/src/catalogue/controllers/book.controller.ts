...
export class BookController {
    ...
    @ApiOperation({ summary: 'List book titles (paginated, searchable, Redis-cached 30s)' })
    @Get('books')
    findAll(
        @Query('page') page = '1',
    ...
    @ApiOperation({ summary: 'Get a book title with all copies (Redis-cached 60s)' })
    @Get('books/:id')
    ...
    @ApiOperation({ summary: 'Delete a book title — only if no copies, bookings, or reviews (Admin/Staff)' })
    @Roles(Role.ADMIN, Role.LIBRARY_STAFF)
    @Delete('books/:id')
    ...
    @ApiOperation({ summary: 'Add a physical copy to a book title (Admin/Staff)' })
    @Roles(Role.ADMIN, Role.LIBRARY_STAFF)
