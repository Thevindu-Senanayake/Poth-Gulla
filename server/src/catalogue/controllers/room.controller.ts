...
export class RoomController {
  ...
  @ApiOperation({
    summary:
      'List study rooms; pass startAt+endAt to get availability flags (Redis-cached 20s if no slot filter)',
  })
  @Get()
  findAll(@Query('startAt') startAt?: string, @Query('endAt') endAt?: string) {
  ...
  @ApiOperation({
    summary: 'Toggle maintenance status on a room (Admin/Staff)',
  })
  @Roles(Role.ADMIN, Role.LIBRARY_STAFF)
