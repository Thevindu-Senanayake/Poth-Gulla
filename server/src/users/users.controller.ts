...
export class UsersController {
  ...
  @ApiOperation({
    summary: 'List users with pagination and filters (Admin/Staff)',
  })
  @Get()
  async findMany(
    ...
    const role =
      roleStr && Object.values(Role).includes(roleStr as Role)
        ? (roleStr as Role)
        : undefined;
    ...
    const search =
      ...
      searchRaw?.replace(/[\x00-\x1F\x7F]/g, '').trim() || undefined;
  ...
  async update(
    ...
    if (!user) throw new NotFoundException('User not found');
    const updated = actor
      ? await this.users.update(id, dto, actor.userId)
  ...
  async disable(
    ...
    if (!user) throw new NotFoundException('User not found');
    const updated = actor
      ? await this.users.setActive(id, false, actor.userId)
  ...
  async enable(
    ...
    if (!user) throw new NotFoundException('User not found');
    const updated = actor
      ? await this.users.setActive(id, true, actor.userId)
