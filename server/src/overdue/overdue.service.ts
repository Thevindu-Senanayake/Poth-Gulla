...
export class OverdueService {
    ...
    async runSweep(): Promise<{ borrowingsProcessed: number; noShowsProcessed: number }> {
    ...
    private async sweepBorrowings(): Promise<number> {
        ...
        const activeBorrowings = await this.prisma.borrowing.findMany({
            ...
            include: { bookCopy: { select: { assetTag: true } }, device: { select: { assetTag: true, name: true } } },
        });
    ...
    private notifyUser(userId: string, type: string, message: string): Promise<unknown> {
        return this.prisma.notification.create({ data: { userId, type, message } });
    }

    private async notifyRole(role: Role, type: string, message: string): Promise<void> {
