...
describe('App (e2e smoke)', () => {
    ...
    describe('Booking', () => {
        ...
        it('POST /api/bookings without a token → 401', () =>
            request(app.getHttpServer()).post('/api/bookings').send(validBooking).expect(401));
        ...
        it('POST /api/bookings as a student → 201 and reaches the service', async () => {
            await request(app.getHttpServer())
                ...
                .send(validBooking)
                .expect(201);
            expect(bookingService.create).toHaveBeenCalledWith('student-1', expect.any(Object));
        });
    });
    ...
    describe('Scan', () => {
        it('POST /api/scan/room-checkin as a student → 201 (+20 pts)', async () => {
            ...
            expect(scanService.roomCheckin).toHaveBeenCalledWith('student-1', expect.any(Object));
        });
